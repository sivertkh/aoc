    ...
    if end is None:
        ...
        for c, part in enumerate(parts):
            if part[0] == 0:
                ...
                del new_parts[c]
                create_bridge([part], new_parts, part[1], largest, lengths)
            elif part[1] == 0:
                new_parts = list(parts)
                del new_parts[c]
                create_bridge([part], new_parts, part[0], largest, lengths)
    else:
        # in bridge
        for c, part in enumerate(parts):
            ...
            if part[0] == end:
                new_parts = list(parts)
                del new_parts[c]
                create_bridge(bridge+[part], new_parts, part[1], largest,
                              lengths)
            elif part[1] == end:
                new_parts = list(parts)
                del new_parts[c]
                create_bridge(bridge+[part], new_parts, part[0], largest,
                              lengths)
        # At the end.
